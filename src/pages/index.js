import React from 'react'
import { useRouter } from 'next/router'
import { Head } from '@/sections'
import { ArrowForward, HorizontalRule } from '@mui/icons-material'
import { Box, Button, Container, Grid, Typography } from '@mui/material'

const index = () => {
  const router = useRouter()
  return (
    <>
      <Head
        title="Aditya Nitin Burse - Expert React & Next.js Developer | Full Stack Developer | SEO Specialist"
        description="Welcome to the portfolio of Aditya Nitin Burse, an award-winning React and Next.js developer. Explore captivating, responsive websites and web applications that stand out in the digital landscape. Let's bring your digital vision to life!"
        robots="index, follow"
        canonical="/"
      />
      <Box className="color-block d-none d-lg-block"></Box>
      <Container maxWidth="xxl">
        <Grid container spacing={2} alignItems={"center"}>
          <Grid item xs={12} md={4} className='d-none d-lg-block home-banner-bg'></Grid>
          <Grid item xs={12} md={7}>
            <Box p={2} className="d-flex justify-content-center d-lg-none">
              <img src="/images/sq.png" className='img-fluid banner-mob-image' />
            </Box>
            <Box className="d-flex flex-column align-items-center align-items-lg-end">
              <Typography variant='h1' className='text-color mb-4 text-center text-right'><span className='text-orange'>.I'm Aditya Burse <HorizontalRule /></span><br />Full Stack Developer</Typography>
              <Typography className='text-color mb-4 text-center text-right'>Crafting captivating websites and web applications<br /> with expertise in React, Next.js, and SEO.<br /> Let's bring your digital vision to life!</Typography>
              <Button className='primary-button' onClick={() => router.push('/about')}>More About Me <ArrowForward className='ms-2' /></Button>
            </Box>
          </Grid>
        </Grid>
      </Container>
    </>
  )
}

export default index